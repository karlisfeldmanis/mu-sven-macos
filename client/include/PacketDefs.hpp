...
struct PMSG_STAT_ALLOC_SEND {
  ...
  uint16_t ag;            // Remaining AG
  uint16_t maxAg;         // Updated maxAg (any stat affects DK AG)
};
...
struct PMSG_SHOP_BUY_RECV {
  ...
  uint8_t targetSlot; // 0xFF = auto, else specific bag slot
};
