...
var _attribute_container: VBoxContainer
...
func _setup_ui():
	...
	scroll.position = Vector2(get_window().size.x - 210, 10)
	canvas.add_child(scroll)
	
	_attribute_container = VBoxContainer.new()
	scroll.add_child(_attribute_container)
	
	var attr_title = Label.new()
	attr_title.text = "Mesh Attributes"
	attr_title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_attribute_container.add_child(attr_title)
	
	var spacer = Control.new()
	spacer.custom_minimum_size = Vector2(0, 10)
	_attribute_container.add_child(spacer)
...
func _process(delta):
	...
	# Animate Materials (Phase 16: Simplified by Shader)
	# Legacy support for UV1 offsets if still using StandardMaterials elsewhere
...
func toggle_wireframe():
	...
	for mesh in _get_all_meshes(_model_container):
		...
		# NOTE: Godot 4 Wireframe is best handled via debug draw modes
		# or a separate wireframe-pass shader. For now, we use a simple override.
		if _show_wireframe:
			mesh.material_override = StandardMaterial3D.new()
			mesh.material_override.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
			mesh.material_override.transparency = BaseMaterial3D.TRANSPARENCY_DISABLED
...
func load_model(path: String, no_skin: bool = false, no_texture: bool = false):
	...
	for c in _attribute_container.get_children():
		if c is VBoxContainer or c is HSeparator or c is Label:
			if c.name != "Title":
				c.queue_free()
...
func _load_bmd(path: String, _no_skin: bool = false, _no_texture: bool = false):
	...
	# Attach Animation Controller for skeletal props
	var velocity = MUModelRegistry.get_velocity(path)
	if not parser.actions.is_empty() and velocity > 0.0:
		var anim_ctrl = MUAnimationController.new()
		anim_ctrl.velocity = velocity
		anim_ctrl.action_index = _current_pose_idx
		skeleton.add_child(anim_ctrl)
	...
	for i in range(parser.meshes.size()):
		...
		if mi:
			...
			# Apply BlendMesh logic (Phase 16: Shader-based)
			...
			if mat is ShaderMaterial:
				mat.set_shader_parameter("albedo_color", Color(light[0], light[1], light[2], alpha))
			
			_add_mesh_debug_ui(i, bmd_mesh, mat)
	
	# Apply Object Effects (Fire, Light, etc.)
	var model_type = MUModelRegistry.get_model_type(path)
	MUObjectEffectManager.apply_effects(_model_container, model_type, 0) # Lorencia = world 0
...
func _add_mesh_debug_ui(idx: int, mesh_data: Variant, mat: Material):
	var panel = VBoxContainer.new()
	_attribute_container.add_child(panel)
	
	var label = Label.new()
	label.text = "Mesh %d: %s" % [idx, mesh_data.texture_filename]
	label.modulate = Color(0.8, 0.8, 1.0)
	panel.add_child(label)
	
	# Shader Control Toggles
	if mat is ShaderMaterial:
		var create_toggle = func(text: String, param: String):
			var cb = CheckBox.new()
			cb.text = text
			cb.button_pressed = mat.get_shader_parameter(param)
			cb.toggled.connect(func(p): mat.set_shader_parameter(param, p))
			panel.add_child(cb)
		
		create_toggle.call("Chrome (SphereMap)", "use_chrome")
		create_toggle.call("Wave (VertexAnim)", "use_wave")
		create_toggle.call("Bright (Additive)", "use_bright")
		create_toggle.call("Pulsing (Window/Candle)", "use_pulsing")
		create_toggle.call("Jitter (Window Flicker)", "use_jitter")
		create_toggle.call("Sawtooth (Fountain)", "use_sawtooth")
