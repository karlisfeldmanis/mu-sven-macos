...
func load_heightmap():
	...
	# 1. Load Data
	var world_data = MUAPI.data().load_world_data(world_id, data_path)
	...
	# 2. Setup Environment
	MUAPI.render().setup_world_environment(self, world_id)
	...
	var mesh = MUAPI.mesh().build_terrain_mesh(_heightmap_data, lightmap)
...
func _setup_material(
	...
	for idx in texture_cache:
		...
		var tex = MUAPI.render().load_mu_texture(ProjectSettings.globalize_path(tex_path))
		if tex:
	...
	var mat = MUAPI.render().create_terrain_material(
