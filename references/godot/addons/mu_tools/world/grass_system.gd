...
func _spawn_multimesh_grass(objects: Array, world_id: int, heightmap: Node3D):
	...
	for bmd_path in buckets:
		...
		if grass_mat is BaseMaterial3D:
		...
		elif grass_mat is ShaderMaterial:
			grass_mat.set_shader_parameter("alpha_scissor_threshold", 0.5)
		...
		for i in range(insts.size()):
			var inst = insts[i]
			var t = Transform3D()
			# SVEN Parity: Grass at 0.5x scale with random 0.8-1.2x variation
			var variation = randf_range(0.8, 1.2)
			var sc_vec = MUCoordinateUtils.convert_scale(inst.scale) * 0.5 * variation
			t = t.scaled(sc_vec)
