...
class BMDMesh:
	...
	# TextureScript Attributes (Extracted from filename)
	var script_bright: bool = false      # _R
	var script_hidden: bool = false      # _H
	var script_stream: bool = false      # _S
	var script_noblend: bool = false     # _N
...
func _parse_meshes(stream: StreamPeerBuffer, _debug: bool) -> bool:
	for mesh_idx in range(header.mesh_count):
		...
		# SVEN TextureScript Parsing Logic (Strict Parity)
		# Attributes like _R (Bright), _H (Hidden) are encoded in the filename.
		# They must follow an underscore and consist ONLY of valid script characters.
		var last_underscore = mesh.texture_filename.rfind("_")
		if last_underscore != -1:
			var base_name = mesh.texture_filename.get_basename().to_upper()
			var suffix = base_name.substr(last_underscore + 1)
			
			# Check if suffix consists only of valid MU script tokens (R, H, S, N)
			# This prevents "tile_house" from being caught as "Hidden"
			var is_valid_script = !suffix.is_empty() and suffix.length() <= 3
			if is_valid_script:
			...
			if is_valid_script:
				if "R" in suffix: mesh.script_bright = true
				if "H" in suffix: mesh.script_hidden = true
				if "S" in suffix: mesh.script_stream = true
				if "N" in suffix: mesh.script_noblend = true
