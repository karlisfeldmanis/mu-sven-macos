	...
	if bake_pose and _parser and not _parser.bones.is_empty():
		...
		for pass_idx in range(16):
			...
			for i in range(_parser.bones.size()):
				...
				# Check if parent is processed (or root)
				if p_idx == -1 or bones_done[p_idx]:
					var raw_pos = bone.position
					var raw_rot = bone.rotation
					
					# Priority: Action 0 for bind pose
					if (action_to_use and action_to_use.keys.size() > i and
						...
						raw_pos = key0.position
						raw_rot = key0.rotation
					
					var local_pos = MUTransformPipeline.local_mu_to_godot(raw_pos)
					var local_rot = MUTransformPipeline.mu_rotation_to_quaternion(raw_rot)
					var local_t = Transform3D(Basis(local_rot), local_pos)
