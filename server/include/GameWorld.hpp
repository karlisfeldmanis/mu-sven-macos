...
public:
    ...
    void Update(float dt, std::function<void(uint16_t)> dropExpiredCallback = nullptr,
                std::vector<MonsterMoveUpdate> *outWanderMoves = nullptr);

    // Check monster aggro/attacks against player positions. Returns attacks to broadcast.
    // Also populates outMoves with position updates for moving monsters.
    std::vector<MonsterAttackResult> ProcessMonsterAI(float dt, const std::vector<PlayerTarget> &players,
                                                       std::vector<MonsterMoveUpdate> &outMoves);

    const std::vector<NpcSpawn> &GetNpcs() const { return m_npcs; }
    const std::vector<MonsterInstance> &GetMonsterInstances() const { return m_monsterInstances; }
    ...
    std::vector<uint8_t> BuildMonsterViewportV2Packet() const; // New 0x34 with HP/state
    ...
    // Monster stats (from Monster.txt Main 5.2 — identical to 0.97k for Lorencia mobs)
    ...
    static constexpr float CHASE_SPEED = 200.0f;       // Chase/return speed (player=334, so player can outrun)
    static constexpr float WANDER_SPEED = 80.0f;       // Slow patrol wander
    static constexpr float LEASH_RANGE = 900.0f;       // Max distance from spawn before returning
    // Per-type constants (atkCooldownTime, wanderRange, aggroRange) are on MonsterInstance
    ...
    static constexpr uint8_t TW_SAFEZONE = 0x01;  // Safe zone — no monster aggro (from _define.h)
