...
| `BMDUtils.hpp` | Bone math: `ComputeBoneMatrices()`, `MuMath::TransformPoint/RotateVector`, AABB. Uses 3x4 row-major matrices. |
| `TextureLoader.hpp` | OZJ/OZT loading, texture resolution, `TextureScriptFlags` (_R/_H/_N suffixes), `TextureLoadResult` (ID + hasAlpha). |
| `MeshBuffers.hpp` | Per-mesh GPU state: VAO/VBO/EBO + rendering flags (hasAlpha, noneBlend, hidden, bright). |
...
| `BMDUtils.cpp` | Euler→quaternion→matrix, parent-chain bone concatenation. |
...
| File | Purpose |
|------|---------|
| `model.vert` | Standard MVP transform, normal correction via inverse-transpose. Inputs: aPos, aNormal, aTexCoord. |
| `model.frag` | Two-sided diffuse lighting (abs dot), alpha discard at 0.1, optional linear fog (2000-8000 range). |
