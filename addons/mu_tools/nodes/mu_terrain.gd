...
const MUPreflight = preload("res://addons/mu_tools/ui/mu_preflight.gd")
...
func _setup_material():
	...
	if map_data:
		for y in range(TERRAIN_SIZE):
			# Disable random offset for now as it causes "striping/shearing" artifacts
			var random_offset = 0.0 # float(randi() % 4) / 4.0
			for x in range(TERRAIN_SIZE):
				var idx = y * TERRAIN_SIZE + x
				# Baseline: Standard Mapping (x, y)
				...
				grass_offset_img.set_pixel(x, y, Color(random_offset, 0, 0))
...
func _create_grass_mesh():
	...
	for grass_type in map_data.grass_tiles.keys():
		...
		mat.shader = load("res://addons/mu_tools/shaders/grass_billboard.gdshader")
...
func _spawn_objects():
	...
	# --- VERIFICATION: Teleport Camera to Bridge01 ---
	# To check if bridge geometry aligns with terrain texture
	for obj_node in get_children():
		if obj_node.name.begins_with("Waterspout01"):
			var cam = get_viewport().get_camera_3d()
			if cam:
				print("[VERIFY] Teleporting Camera to Fountain (Water): ", obj_node.global_position)
				cam.global_position = obj_node.global_position + Vector3(0, 15, 15) # Close-up Angle
				cam.look_at(obj_node.global_position)
				break
...
func get_height_at_world(world_pos: Vector3) -> float:
	...
	# SVEN Canonical Sync (Transposed): Godot Z -> MU Column (X), Godot X (mirrored) -> MU Row (Y)
	var mu_x = world_pos.z
	var mu_y = 256.0 - world_pos.x
	...
	# Sample 4 corners
	# SVEN Parity: Heightmap is stored in ROW-MAJOR order: idx = y * SIZE + x
