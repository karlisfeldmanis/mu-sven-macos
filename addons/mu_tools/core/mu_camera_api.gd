...
func focus_on_mu_coordinate(rig: Node3D, mu_x: float, mu_y: float):
	const MUCoordinateAPI = preload("res://addons/mu_tools/core/mu_coordinate_api.gd")
	var coords = MUCoordinateAPI.new()
	var target = coords.mu_to_godot(Vector3(mu_x, mu_y, 0))
